...
class Portal extends TagLib
{
    ...
    protected $tags = [
        ...
        'newlist'          => ['attr' =>'field,order,limit,like,sql,name,where,page','close' =>1,],
        'custom'           => ['attr' =>'field,id,categoryIds,pageVarName,returnVarName','close'=>1]
    ];
    /**
     * 自定义表标签
     */
//    public function tagCustom($tag,$content){
//        $item = empty($tag['item']) ? 'v' : $tag['item'];//循环变量名
...
//        }
     ...
     * $parse = <<<parse
    <?php
    \${$returnVarName} = \app\portal\service\ApiService::custom([
    ...
    ?>
    <volist name="{$returnVarName}.custom" id="{$item}">
    {$content}
    </volist>
    parse;
    return $parse;
